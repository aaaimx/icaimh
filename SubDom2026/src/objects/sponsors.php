<?php
$sponsors = [
  [
    "name" => "Tecnológico Nacional de México",
    "img" => "/public/img/TECNM.png",
    "url" => "https://www.tecnm.mx/"
  ],
  [
    "name" => "AAAIMX",
    "img" => "/public/img/AAAI.png",
    "url" => "https://www.aaaimx.org/"
  ],
  [
    "name" => "Thomson Rivers University",
    "img" => "/public/img/tru.png",
    "url" => "https://www.tru.ca/"
  ],
  [
    "name" => "Cátedra Extraordinaria BOSCH UCM en Inteligencia Artificial aplicada a Internet de las Cosas",
    "img" => "/public/img/bosch.png",
    "url" => "https://gaia.fdi.ucm.es/research/catedraBosch/"
  ],
  [
    "name" => "Universidad Complutense de Madrid",
    "img" => "/public/img/UCM.png",
    "url" => "https://www.ucm.es/"
  ],
  [
    "name" => "Universidad Nacional Autónoma de México",
    "img" => "/public/img/unam.png",
    "url" => "https://unam.mx/"
  ],
  [
    "name" => "Cinvestav",
    "img" => "/public/img/CINV800x600.png",
    "url" => "https://www.cinvestav.mx/"
  ],
  [
    "name" => "Association for the Advancement of Artificial Intelligence",
    "img" => "/public/img/aaai-transpeps.png",
    "url" => "https://www.aaai.org/"
  ],
  [
    "name" => "Instituto Tecnológico de Tijuana",
    "img" => "/public/img/itt.png",
    "url" => "https://tijuana.tecnm.mx"
  ],
  [
    "name" => "Instituto Politécnico Nacional",
    "img" => "/public/img/IPN800x600.png",
    "url" => "https://www.ipn.mx/"
  ],
  [
    "name" => "Instituto Tecnológico de León",
    "img" => "/public/img/itl.png",
    "url" => "https://leon.tecnm.mx"
  ],
  [
    "name" => "Ozelot",
    "img" => "/public/img/ozelot.png",
    "url" => "https://ozelot.mx"
  ],
  [
    "name" => "Brain House",
    "img" => "/public/img/brainhouse.png",
    "url" => "https://brainhouse.mx"
  ],
  [
    "name" => "Tecnológico de Monterrey",
    "img" => "/public/img/tm.png",
    "url" => "https://tec.mx"
  ],
  [
    "name" => "Association for Computing Machinery",
    "img" => "/public/img/acm.png",
    "url" => "https://www.acm.org/"
  ],
  [
    "name" => "JARKOL Technologies",
    "img" => "/public/img/JAR.png",
    "url" => "https://www.jarkol.com/"
  ],
  [
    "name" => "Distributed Analytics",
    "img" => "/public/img/da.png",
    "url" => "https://distributedanalytics.co.uk"
  ],
  [
    "name" => "Institute for the Future of Education",
    "img" => "/public/img/ife.png",
    "url" => "https://tec.mx/en/ife?srsltid=AfmBOor-mflpVavLp0Xg6XSy42hAOzsKSveWRcSgV-LCK5CkgY5MJAMW"
  ],
  [
    "name" => "R4C-IRG Interdisciplinary Research Group",
    "img" => "/public/img/r4c.jpeg",
    "url" => "https://tec.mx/en/r4c-irg?srsltid=AfmBOoro0M6EHj4Qj6M7YbZhOdAVdfn8P8Ob-hjpue09b7us5_-7zWHL"
  ],
  [
    "name" => "ITM ACM Student Chapter",
    "img" => "/public/img/itm-acm.png",
    "url" => "https://itm-acm.mx"
  ],
  [
    "name" => "Mirrortrec",
    "img" => "/public/img/Mirrortrec.jpg",
    "url" => "https://www.facebook.com/share/1BWvsSuqg4/"
  ]
];
